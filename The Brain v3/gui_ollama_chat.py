...
class OllamaGUI:
    ...
    # Model management controls (pull/refresh/remove) moved to Settings tab
        ...
        ttk.Label(ctrl_frame, text='Max chars B').grid(row=2, column=0, sticky='w')
        self.max_chars_b = tk.IntVar(value=120)
        ttk.Spinbox(ctrl_frame, from_=0, to=10000, textvariable=self.max_chars_b, width=7).grid(row=2, column=1)
        self.short_turn_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(ctrl_frame, text='Short-turn', variable=self.short_turn_var).grid(row=1, column=4, padx=6)
        self.log_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(ctrl_frame, text='Log to file', variable=self.log_var).grid(row=1, column=5, padx=6)
        self.log_path = ttk.Entry(ctrl_frame, width=30)
        self.log_path.insert(0, '')
        self.log_path.grid(row=1, column=6, sticky='w')
        self.close_on_exit_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(ctrl_frame, text='Close terminal on exit', variable=self.close_on_exit_var).grid(row=1, column=7, padx=6)
        self.start_btn = ttk.Button(ctrl_frame, text='Start', command=self.start)
        self.start_btn.grid(row=1, column=3, padx=6)
        ...
        self.reset_btn.grid(row=1, column=5, padx=6)
        # Save & Exit button
